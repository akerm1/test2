...
signInForm.addEventListener('submit', (e) => {
    ...
    signInWithEmailAndPassword(auth, email, password)
        .then((userCredential) => {
            ...
            window.location.href = 'https://splendid-tartufo-9b7b3e.netlify.app/';  // Redirect after successful sign-in
        })
        .catch((error) => {
            console.error(error.message);
...
document.getElementById('googleSignInButton').addEventListener('click', () => {
    ...
    .then((result) => {
        ...
        signInStatus.style.color = 'green';
        window.location.href = 'https://splendid-tartufo-9b7b3e.netlify.app/';  // Redirect after successful Google Sign-In
    })
    .catch((error) => {
